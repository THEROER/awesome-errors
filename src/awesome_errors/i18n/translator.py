...
# Built-in English defaults. Kept in memory (not written to disk) so importing
# the library never mutates the installed package or the working directory.
...
class ErrorTranslator:
    ...
    def __init__(self, locales_dir: Optional[Path] = None, default_locale: str = "en"):
        ...
        # Seed English defaults in memory; any on-disk locale files (including a
        # bundled ``en/errors.json``) are loaded on top and take precedence.
        self._translations: Dict[str, Dict[str, str]] = {"en": dict(DEFAULT_TRANSLATIONS)}
        self._load_translations()

    def _load_translations(self) -> None:
        """Load translation files from disk, if the directory exists."""
        ...
        for locale_file in self.locales_dir.glob("*/errors.json"):
            ...
            try:
                ...
                continue
            self._translations.setdefault(locale, {}).update(loaded)
    ...
    def translate(
        ...
        message = self._translations.get(locale, {}).get(error_code)
        if message:
            return self._format_message(message, params)

        # Fall back to English, but only for locales that aren't English
        # themselves (case-insensitive) so an unknown "EN" doesn't silently
        # resolve via the "en" table.
        if locale.lower() != "en":
            message = self._translations.get("en", {}).get(error_code)
            if message:
                return self._format_message(message, params)

        # No translation found — return the raw code.
        return error_code

    def _format_message(self, message: str, params: Optional[Dict[str, Any]]) -> str:
        """Format message with parameters, ignoring missing/extra keys."""
        if not params:
            return message
        try:
